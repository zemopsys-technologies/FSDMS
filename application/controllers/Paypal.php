...
// the above types and intent set to sale 'sale'
    ...
    function success()
    {
        ...
        {
            $set_update_on_sku_rowid_userid = $this->update_model->set_flag_product_cart($arraySku[$x],$arrayRowid[$x],$arrayOrderId[$x],$get_user_id);
            if($set_update_on_sku_rowid_userid)
            {
                ...
                $get_affected_cart_confirmation = $this->update_model->set_flag_cart_confirmation($arraySku[$x],$arrayRowid[$x],$arrayOrderId[$x],$get_user_id,$str_from_date,$str_to_date);
                ...
                if($get_affected_cart_confirmation)
                {
                    ...
                    {
                        $branch_address = $arrayBranchAddress[$i];
                    }
                    $set_address = array(
        ...
        if($get_update_counts)
        {
            ...
            echo "<script>setTimeout(function(){window.location.href='".base_url()."home';},3000);</script>";
        }
