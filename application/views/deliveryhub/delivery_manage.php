                        ...
                        <select class="form-control" name="product_sku[]" id="p_id">
                               ...
                               <?php  $z++; ?>
                                <option value="<?php echo  $row->product_sku.'%'. $row->order_id;?>">
                                    <?php echo $row->product_sku.'%'. $row->order_id;?>
                                </option>
                                <?php 
                                } 
                                ?>
...
if(x < max_fields){ //max input box allowed
    ...
    $(wrapper).append('<div class="row"><div class="card-body col-md-3" align="right"></div><div class="col-md-6"><input type="hidden" name="count'+x+'" value="'+x+'" ><select class="form-control dropdown" name="product_sku[]" data-count="'+x+'" id="p_id'+x+'"><option value=" "> Select Kitchen </option><?php foreach($product_data->result() as $row) {?><option value="<?php echo  $row->product_sku.'%'. $row->order_id;?>"><?php echo $row->product_sku.'%'. $row->order_id;?></option><?php  } ?></select><input type="hidden" id="pro_id'+x+'" name="product_name[]" value=""/><input type="hidden" id="pro_sku'+x+'" name="prod_sku[]" value=""/><input type="hidden" id="pro_rowid'+x+'" name="rowid[]" value=""/><input type="hidden" id="pro_quantity'+x+'" name="quantity[]" value=""/><input type="hidden" id="pro_user'+x+'" name="userid[]" value=""><input type="hidden" id="pro_order'+x+'" name="order_id[]" value=""><a href="#" class="remove_field1">REMOVE</a></div></div>'); 
}
});
